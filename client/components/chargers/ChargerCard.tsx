  ...
  return (
    <View className="mb-4 rounded-2xl border border-gray-200 bg-white p-4">
      ...
      <View className="flex-row items-start justify-between">
        ...
        <View
          className={`flex-row items-center rounded-full px-3 py-1 ${status.bg}`}
        >
          <View
            className={`mr-2 h-2 w-2 rounded-full ${status.dot}`}
          />
      ...
      {charger.status === "available" && (
        <TouchableOpacity onPress={handleBookSlot}>
          <Text className="mt-4 self-end font-medium text-gray-500">
            Book slot →
          </Text>
        </TouchableOpacity>
